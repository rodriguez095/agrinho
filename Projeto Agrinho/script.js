...
function atualizarReactionCounts() {
  const likeCount = document.getElementById('likeCount');
  const heartCount = document.getElementById('heartCount');

  if (likeCount) {
    likeCount.textContent = reactionState.like;
  }

  if (heartCount) {
    heartCount.textContent = reactionState.heart;
  }
}
  ...
  document.querySelectorAll('.reaction-btn').forEach((button) => {
    button.addEventListener('click', () => {
      ...
      if (button.classList.contains('like-btn')) {
        reactionState.like += isActive ? -1 : 1;
      }

      if (button.classList.contains('heart-btn')) {
        reactionState.heart += isActive ? -1 : 1;
      }

      atualizarReactionCounts();
    });
  });
